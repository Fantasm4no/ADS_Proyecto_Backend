...
exports.getHistorialByCliente = async (req, res) => {
  ...
  try {
      ...
      // Obtener el historial de compras con imagen de productos y membresías
      const historial = await pool.query(
          ...
          SELECT h.*, 
                 ...
                 m.imagen_url AS membresia_imagen
      ...
      // Mapear los resultados para incluir la imagen de la membresía si existe
      const historialEnriquecido = historial.rows.map((item) => ({
          fecha: item.fecha,
          nombre: item.producto_nombre || item.membresia_nombre || "N/A",
          imagen: item.producto_imagen || item.membresia_imagen || null, // Aquí agregamos la imagen de membresía
